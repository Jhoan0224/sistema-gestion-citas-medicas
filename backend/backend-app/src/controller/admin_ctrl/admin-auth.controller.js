...
import * as adminSercuritySvc from '../../services/admin_svc/admin-security.svc.js';
...
export const adminLoginCtrl = async (req, res) => {
    try {
        const processResult = await adminSercuritySvc.loginAdmin(req.body);
        ...
        console.error("Error en controller adminLoginCtrl >> " + error);
        return res.status(500).json(SERVER_ERROR);
    }
};